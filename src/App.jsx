...
const Api_key = '9227c05348e4f819dcf023d3de19fb1a';
...
const App = () => {
  ...
  const fetchWeather = async () => {
    const URL = `https://api.openweathermap.org/data/2.5/weather?q=${inputRef.current.value}&units=metric&lang=id&appid=${Api_key}`;
    setLoading(true);
    fetch(URL)
      .then((res) => res.json())
      .then((data) => {
  ...
  return (
    <div className='bg-neutral-100 h-screen grid place-items-center'>
      <div className='bg-teal-500 w-96 p-4 rounded-md'>
        <div className='flex items-center justify-between'>
          <input
            type='text'
            ref={inputRef}
            placeholder='Ketik disini ya nama kotanya'
            className='text-sm border-collapse text-white
          p-1 border-gray-200 bg-teal-500 font-semibold uppercase flex-1'
          />
        ...
        >
          {loading ? (
            ...
            showWeather && (
              <div className='text-center flex flex-col gap-6 mt-10 text-teal-50'>
